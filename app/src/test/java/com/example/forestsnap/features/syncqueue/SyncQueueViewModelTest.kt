...
class SyncQueueViewModelTest {
    ...
    fun `pendingQueue exposes data from repository`() = runTest {
        val mockData = listOf(
            SyncSnapEntity(id = 1, photoPath = "path1", latitude = 0.0, longitude = 0.0, timestamp = 0L),
            SyncSnapEntity(id = 2, photoPath = "path2", latitude = 0.0, longitude = 0.0, timestamp = 0L)
        )
        ...
        viewModel.pendingQueue.test {
            assertEquals(emptyList<SyncSnapEntity>(), awaitItem()) // Initial state
            assertEquals(mockData, awaitItem()) // Emitted state
